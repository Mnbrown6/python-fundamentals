...
a single quote or double quote and can be letters of number or characters
"""
alpha = 'Hello'
apple: str = 'Red'
...
This does not include empty spaces, but not numbers.
"""
...
to be combine with each other using curly braces as placeholders
"""


def basic_string_format(value: int):
    msgl: str = 'My dog is {} years old'
    print(msgl.format(value))
...
""" The f-string uses expressions within the string directly. 
It also can use the same mini-programming as the format function.
Essentially the interpreter runs the format function itself.
The f-string also uses place holder curly braces
"""
...
def basic_fstring(name: str, snack: str, num_snacks: int):
    ...
    like: str = f'I like {snack} type of snacks'
    print(like)


basic_fstring('Mahasin','Chocolate', 5)

""" String methods are methods built-in to strings to create
new values of the string. There are more than what we will 
write below.
"""
